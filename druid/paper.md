...
### 2.1 实时任务  
实时任务进程由容器节点调起，但实际由管理节点管理。管理节点接收到需要接入数据请求后，根据请求的描述依据事先配置的调度策略来调度不同的容器启动相应的实时任务。用户可以根据需要自由选择调度策略，系统默认为公平调度，优先选择空闲容量较大的容器节点。管理节点则会轮询查个各个任务的状态，一旦任务状态失败，则会自动重新选择容器节点调启任务。  
...
针对CPU聚合慢、性能差的问题，我们通过以下两种方式来解决：    
* 所有聚合函数统一使用C++使用，由JavaCPP调用，性能提升一倍以上
* 利用向量化技术，提供CPU计算并行度，大大提升CPU性能，一般情况可以提升两倍以上  

经过一系列优化，Tindex单机每秒聚合数据可达2亿行以上。相对其他产品，性能有绝对优势。  

## 5 与大数据生态圈结合  

Tindex为了更方便使用，已经对接了Hive和Spark。Hive和Spark只需要引入我们的扩展包，可以直接查询查询Tindex，另外还支持CTAS(Create Table As Select)的方式创建Tindex数据源，另外还支持Insert override into table的方式去覆盖已存在的数据。另外，我们还在积极地与生态圈的其他技术对接，
如下：
【图片】
